...
- **Evidence provenance** — every `evidence_plan[*]` entry with `status: collected` carries a `collected_at` timestamp; the timestamp must be greater than or equal to the manifest's earliest `last_updated` for the change. When the artefact's content embeds a commit SHA, version string, or CI run ID, that identifier should be reachable from the change's commit history (the artefact was captured against a commit that is part of this change, not a sibling or unrelated commit). A row marked `collected` whose `collected_at` predates the manifest's creation, or whose embedded identifier resolves to an unrelated commit, is a Tier 2 finding (advisory severity by default; runtime bridges may opt to block). The check is the audit-side counterpart of [`docs/repo-as-context-discipline.md`](repo-as-context-discipline.md) §Rule 1: evidence that does not provably come from the change is not evidence of the change. See [`docs/automation-contract-algorithm.md`](automation-contract-algorithm.md) §Rule 2.13 for the algorithm.
