...
| [`autonomy-ladder-discipline.md`](autonomy-ladder-discipline.md) | The five-rung autonomy ladder (L0 hand-written → L1 draft-assist → L2 in-loop → L3 cross-Phase → L4 end-to-end). Each rung's minimum evidence shape and HITL trigger; rung × surface table; upgrade procedure. Indexes existing rules by autonomy level; introduces no new permission tier. |
| [`observability-legibility-discipline.md`](observability-legibility-discipline.md) | Three rules keeping the running system's operational evidence reachable by an agent in-loop: structured-by-default emission, per-change isolation, query-by-capability (log/metric/trace query as capability categories, not backends). The operational-surface counterpart of [`repo-as-context-discipline.md`](repo-as-context-discipline.md). |
| [`throughput-first-merge-philosophy.md`](throughput-first-merge-philosophy.md) | Three rules for the merge-gate posture when agent throughput exceeds human attention: default-advisory, short-lived PRs, follow-up over indefinite blocking. Strict gate (Tree D leaves, L2+ breaking changes, mode-3 rollback, auth/PII/secrets) overrides the philosophy. |
| [`file-role-map.md`](file-role-map.md) | Per-file role classification (SoT / thin-bridge / runtime role-spec / reference wrapper) and the "where a new rule belongs" routing. Lifted from [`AGENTS.md`](../AGENTS.md) §File role map in 1.31.0 so the entry point stays a table of contents. |
