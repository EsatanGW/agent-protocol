...
> **English TL;DR**
> Vendor-neutral specification of *where* the methodology can be wired into an automation pipeline and *what each hook should check* — without binding to any specific CI/CD product. Defines seven hook points: (1) PR create → surface-label check, (2) PR create → cross-cutting checklist reminder, (3) PR update → docs-sync check, (4) PR update → SoT-change warning, (5) PR review → evidence attachment check, (6) pre-merge → completion-report check (Full-mode PRs), (7) post-merge → Phase 8 observation reminder. Each hook specifies: trigger event, input, expected output, advisory-vs-blocking policy. Change Manifest schema validation itself is a separate concern handled by the automation contract's Layer 1 (`automation-contract-algorithm.md`), not a hook in this list. Principle: methodology stays usable *without* any hooks; automation is an accelerator, not a gate. Pairs with `automation-contract-algorithm.md` (the normative algorithm) and `reference-implementations/` (non-normative example validators).
