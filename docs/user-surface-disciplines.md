...
> **English TL;DR**
> Per-change checklist for the *user surface* — what a human actually sees, clicks, waits for, and interprets. Observation axes: **flow correctness** (can the user complete the requested journey end-to-end), **contract alignment** (component data shape matches SoT; loading/empty/error/partial states all handled), **feedback + guidance** (success/failure/pending all visible; copy matches i18n source of truth), **a11y** (keyboard, focus, semantic structure, contrast), **responsive + device** (target viewports, dark/light, reduced motion), **analytics + telemetry** (events fire at the right moments with stable names). Applies to route / page / component / form / table / modal / drawer / state store / query layer / i18n / design system changes. Companion to `implementation-disciplines.md` and `operational-disciplines.md`.
