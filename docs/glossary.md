...
CCKNs have asymmetric timing: **write at Phase 4 Discovery / Phase 1 re-entry** (when a change produces reusable knowledge — §Relation to Change Manifest §3 and §Ceremony scaling in the full definition); **query at Phase 1 Investigate startup** (before tracing the main flow, grep the CCKN directory for topics overlap with the change's anticipated surfaces / uncontrolled_interfaces / libraries — §When to query a CCKN). Querying late is the primary anti-pattern — decisions the CCKN should have informed have already been made.
