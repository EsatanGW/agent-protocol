#!/usr/bin/env sh
# Check that the version string is consistent across the four places
...
#
# Drift between these four is the single most common release-hygiene bug.
...
if [ $fail -eq 0 ]; then
  echo "OK: all four declarations agree on $plugin_version"
fi
exit $fail
