...
| # | Case | Asserts |
|---|------|---------|
| 1 | `unset-env-yields-empty-ap-vars` | When `GEMINI_HOOK_EVENT` / `GEMINI_TOOL_NAME` / `AGENT_PROTOCOL_MANIFEST_PATH` are all unset, every `AP_*` is empty (no crash) |
| 2 | `runtime-env-maps-to-event-and-tool` | `GEMINI_HOOK_EVENT=post-tool-use` → `AP_EVENT=post-tool-use`; `GEMINI_TOOL_NAME=edit` → `AP_TOOL=edit` |
| 3 | `manifest-yaml-populates-phase` | When `yq` is on PATH and `AGENT_PROTOCOL_MANIFEST_PATH` points at a YAML with `phase: review`, `AP_PHASE=review`. Skipped when `yq` is missing |
...
- Whether Gemini CLI actually invokes `parse-event.sh` at the right
