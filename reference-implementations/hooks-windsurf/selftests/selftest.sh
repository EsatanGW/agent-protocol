#!/usr/bin/env sh
# Hermetic selftest for the Windsurf event adapter.
#
...
#
# Does NOT test whether Windsurf actually invokes the adapter at the right
...
invoke_adapter() {
  (
    unset WINDSURF_HOOK_EVENT WINDSURF_TOOL_NAME AGENT_PROTOCOL_MANIFEST_PATH 2>/dev/null || true
    for kv in "$@"; do
      key=${kv%%=*}
      val=${kv#*=}
      eval "export $key=\"\$val\""
...
check_case "runtime-env-maps-to-event-and-tool" "post-tool-use" "edit" "" \
  "WINDSURF_HOOK_EVENT=post-tool-use" "WINDSURF_TOOL_NAME=edit"

if command -v yq >/dev/null 2>&1; then
  tmpmf=$(mktemp)
  printf 'phase: plan\nsurfaces_touched: []\n' > "$tmpmf"
  check_case "manifest-yaml-populates-phase" "" "" "plan" \
