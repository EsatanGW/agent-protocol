#!/usr/bin/env sh
# Hermetic selftest for the Cursor event adapter.
#
...
#
# Does NOT test whether Cursor actually invokes the adapter at the right
...
# Invokes the adapter in a clean subshell after unsetting runtime env and
# setting the env assignments passed as arguments ("KEY=VALUE" each).
# Writes "AP_EVENT|AP_TOOL|AP_PHASE|AP_STAGED_FILES" to stdout.
invoke_adapter() {
  (
    unset CURSOR_HOOK_TRIGGER CURSOR_TOOL AGENT_PROTOCOL_MANIFEST_PATH 2>/dev/null || true
    for kv in "$@"; do
      key=${kv%%=*}
      val=${kv#*=}
      export "$key=$val"
...
check_case "runtime-env-maps-to-event-and-tool" "pre-commit" "Edit" "" \
  "CURSOR_HOOK_TRIGGER=pre-commit" "CURSOR_TOOL=Edit"

if command -v yq >/dev/null 2>&1; then
  tmpmf=$(mktemp)
  printf 'phase: implement\nsurfaces_touched: []\n' > "$tmpmf"
  check_case "manifest-yaml-populates-phase" "" "" "implement" \
