#!/usr/bin/env sh
# Hermetic selftest for the Codex event adapter.
#
...
#
# Does NOT test whether Codex actually invokes the adapter at the right
...
invoke_adapter() {
  (
    unset CODEX_HOOK_EVENT CODEX_TOOL_NAME AGENT_PROTOCOL_MANIFEST_PATH 2>/dev/null || true
    for kv in "$@"; do
      key=${kv%%=*}
      val=${kv#*=}
      export "$key=$val"
...
check_case "runtime-env-maps-to-event-and-tool" "pre-commit" "edit" "" \
  "CODEX_HOOK_EVENT=pre-commit" "CODEX_TOOL_NAME=edit"

if command -v yq >/dev/null 2>&1; then
  tmpmf=$(mktemp)
  printf 'phase: signoff\nsurfaces_touched: []\n' > "$tmpmf"
  check_case "manifest-yaml-populates-phase" "" "" "signoff" \
