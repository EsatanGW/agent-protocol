...
| Phase | Scope | Artifact(s) | Gate verification | Status | Commit | Notes |
|---|---|---|---|---|---|---|
| P0 | Open this ROADMAP entry | `ROADMAP.md` new active-initiative row with phase table | Initiative section renders with schema fields populated; driver + scope documented | ⏳ in_progress | _(this change)_ | Opens before any implementation per `phase-gate-discipline.md` |
| P1 | Write iOS/Swift stack bridge | `docs/bridges/ios-swift-stack-bridge.md` covering UIKit + SwiftUI dual UI story, Core Data + CloudKit + UserDefaults + Keychain SoT map, WidgetKit / App Intents / ShareExtension as multi-process state-sharing pipeline-order cases, HealthKit / StoreKit / Sign-in-with-Apple as uncontrolled interfaces, TestFlight + App Store Connect + phased-rollout release mechanics; `docs/bridges/ios-swift-surface-map.yaml` validated against `schemas/surface-map.schema.yaml` | Bridge parses as markdown; surface-map passes `validate-templates.py`-style schema check; bridge's "Known limitations" section cross-links to `bridges-local-deviations-template.md` per 1.3.0 pattern | ⏳ pending | _(pending)_ | iOS is the bridge most frequently asked for; SwiftUI + UIKit interop is the single load-bearing discipline the bridge has to capture |
| P2 | Write React/Next.js stack bridge | `docs/bridges/react-nextjs-stack-bridge.md` covering App Router + RSC + Server Actions dual-render boundary, React state + TanStack Query + server-state drift, `next.config.js` + `middleware.ts` + route handlers as pipeline-order, i18n (`next-intl` / `next-i18next`), edge vs node runtime split, CDN cache + ISR + bfcache + service worker as rollback surface, client bundle + RSC payload size budgets; `docs/bridges/react-nextjs-surface-map.yaml` validated against `schemas/surface-map.schema.yaml` | Bridge parses as markdown; surface-map schema-valid; bridge explicitly calls out the "server/client component boundary = SoT pattern 4a pipeline-order" framing | ⏳ pending | _(pending)_ | Next.js App Router is the most common post-2024 adoption path; covering both App Router + Pages Router would double the bridge length for diminishing returns — App Router only, with Pages Router as a local-overlay pointer |
| P3 | Write iOS/Swift worked example | `docs/examples/ios-swift-app-example.md` — scenario: add a CloudKit-synced tag feature to a Core Data-backed journaling app with a companion Widget, exercising Core Data lightweight migration (SoT pattern 1), CloudKit public schema vs private database (SoT pattern 4 + rollback mode 3 asymmetry between private-db forward-fix and public-schema compensation), App Group UserDefaults for Widget (SoT pattern 8 dual-representation), and phased TestFlight rollout | Example follows the existing 150–250 line worked-example shape (compare `flutter-app-example.md`); ends with a "What this example is meant to show" section listing 3–4 bullets | ⏳ pending | _(pending)_ | The asymmetric rollback between CloudKit private (per-user, forward-fixable) and public (shared, compensation-only) is the most instructive iOS-specific case |
| P4 | Write React/Next.js worked example | `docs/examples/react-nextjs-app-example.md` — scenario: migrate a Server Action form handler from Pages Router API route to App Router Server Action, with CDN-ISR cache invalidation and a middleware rewrite for A/B testing; exercises server/client boundary drift (SoT pattern 4a), stale RSC payload handling, cache-purge rollback (mode 1 CDN flip) vs. already-served-stale-HTML (mode 2 forward-fix) | Example follows the existing worked-example shape; covers Server Component → Client Component boundary as a concrete drift case with a fix | ⏳ pending | _(pending)_ | Server Action migration is the canonical "I moved a handler and now stuff breaks" Next.js case; excellent vehicle for showing boundary-as-contract discipline |
| P5 | Update cross-references | `README.md` repository layout tree; `README.md` "When your situation matches" bullets; `docs/onboarding/orientation.md` "Navigation map" bridge list; `AGENTS.md` "Recommended reading order" bridge list (if it lists them individually) | All links resolve; `grep -r "docs/bridges/" --include="*.md"` produces no broken references | ⏳ pending | _(pending)_ | Index hygiene — new bridges must be discoverable from every page that lists bridges |
| P6 | Release v1.5.0 | `.claude-plugin/plugin.json` + `marketplace.json` + README badge `1.4.0 → 1.5.0`; `CHANGELOG.md` `[1.5.0] - 2026-04-20` entry; this ROADMAP initiative closed and moved to Closed section | `sh .github/scripts/check-version-consistency.sh` reports `OK: all four declarations agree on 1.5.0`; all self-validation scripts pass | ⏳ pending | _(pending)_ | Standard minor-release procedure per `VERSIONING.md` |
